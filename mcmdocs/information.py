...
attr_codes = set(df_census_list[-1].columns).union(
    set(df_census_list[0].columns))
s_list.append(
    pd.Series([attr_dict.get(attr_code) for attr_code in attr_codes],
              index=attr_codes))
...
df_result = pd.DataFrame(
    {str(result_col_names[i]): s_list[i]
     for i in range(8)})
